...
- [ ] 安全性优化
  - [ ] 1.1 登录加密算法 - 使用 .NET 内置 AES 替换自定义 DES 实现
  - [ ] 1.2 JWT 密钥管理 - 使用环境变量或密钥管理服务
  - [ ] 1.3 SQL 注入风险 - 优化 HomeController.DoInstall 中的 SQL 执行方式

- [ ] 性能优化
  - [ ] 2.1 缓存策略 - 统一缓存访问接口，优化缓存过期策略
  - [ ] 2.2 数据库查询优化 - 优化 ArticleController.Index 和 ProductController.Index 的分页查询
  - [ ] 2.3 内存管理 - 移除 IndexController.DoMemoryFree 中的 SetProcessWorkingSetSize 调用

- [ ] 代码质量与可维护性
  - [ ] 3.1 重复代码 - 提取公共方法处理栏目和详情查找逻辑
  - [ ] 3.2 魔法字符串和数字 - 使用常量或枚举替代魔法值
  - [x] 3.3 异常处理 - 为关键操作添加异常处理机制
  - [x] 3.4 配置管理 - 将硬编码的配置项移至 appsettings.json

- [ ] 架构与设计
  - [ ] 4.1 控制器职责 - 拆分 HomeController 的功能到不同控制器
  - [ ] 4.2 业务逻辑与数据访问 - 封装业务逻辑到独立服务类

- [ ] 用户体验优化
  - [ ] 5.1 响应式设计 - 考虑使用成熟 CSS 框架改进响应式设计
  - [ ] 5.2 错误处理页面 - 设计更友好的错误页面

- [ ] 其他建议
  - [ ] 6.1 日志记录 - 增强关键业务操作和异常处理的日志记录
  - [ ] 6.2 单元测试 - 为关键业务逻辑和控制器添加单元测试
  - [ ] 6.3 API 文档 - 提供完整的 API 文档
